...
parser.add_argument(
    ...
    type=str,
    default="surgical",
    choices=["surgical","surgical_blue", "N95", "KN95", "cloth", "gas", "inpaint", "random", "all"],
...
args.bwrite_path = args.path + "_binarymask"
...
if is_directory:
    ...
    for f in tqdm(files):
        ...
        bwrite_path = path + "_binarymask"
        if not os.path.isdir(bwrite_path):
            os.makedirs(bwrite_path)
        ...
        if is_image(image_path):
            ...
            #binary masks
            for i in range(len(mask_binary_array)):
                w_path = (
                    bwrite_path
                    + "/"
                    + split_path[0]
...
# Process if the path was a file
    ...
    if is_image(image_path):
        ...
        for i in range(len(mask)):
            ...
            img = masked_image[i]
            cv2.imwrite(w_path, img)
            ww_path = write_path + "_" + mask[i] + "binary." + args.path.rsplit(".")[1]
            imgmask = mask_binary_array[i]
            cv2.imwrite(ww_path, imgmask)
else:
